...
````carousel
**Original vs. Reconstruction (Epoch 40)**
![Reconstruction Quality](outputs/presentation_assets/reconstruction_epoch_0040.png)

> [!NOTE]
> Notice how the reconstruction captures the overall road structure and nearby vehicles clearly, but distant objects (small cars far ahead) become blurry. This is expected—the VAE prioritizes information critical for immediate driving decisions.
<!-- slide -->
**Latent Space Sampling**
![Random samples from the learned latent space](outputs/presentation_assets/samples_epoch_0040.png)

These are images generated by sampling random points from the latent space $z \sim \mathcal{N}(0, I)$. The fact that they look like plausible road scenes proves the VAE has learned a meaningful, structured representation.
````

**Critical Analysis**: 
- ✅ **Success**: Sharp reconstruction of road, lane markings, and nearby vehicles.
- ⚠️ **Limitation**: Distant objects are blurred or disappear. For immediate driving, this is acceptable, but long-range planning would require higher-resolution encoding.
